...
def createLobby():
    ...
    key_locations.append(key_location + '_' + token)
    spy_amount.append(int(data[2]))
    players_amount.append(int(data[1]))
    spy_amount_realtime.append(0)
    players_amount_realtime.append(0)
    ...
    if role == 1:
        ...
        spy_amount_realtime[-1] += 1
    elif role == 0:
        token_role_keyLocation_locations.append('peaceful')
        players_amount_realtime[-1] += 1
...
def connectToLobby(data):
    ...
    players_amount[tokens.index(data)]
    key_location_current = key_locations[tokens.index(data)].split('_')[0]

    # Генерируем роль мазефакеру и добавляю его в посылку
    role = random.randint(0,1)
    role_keyLocation_locations = []
    if role == 1 and spy_amount_realtime[tokens.index(data)] < spy_amount[tokens.index(data)]:
        role_keyLocation_locations.append('spy')
        spy_amount_realtime[tokens.index(data)] += 1
        players_amount_realtime[tokens.index(data)] += 1
    elif players_amount_realtime[tokens.index(data)] < players_amount[tokens.index(data)]:
...
def isCorrectLocation(data):
    ...
    if data[1] in tokens and ''.join(data[0] + '_' + data[1]) in key_locations and tokens.index(data[1]) == key_locations.index(''.join(data[0] + '_' + data[1])):
